...
import { calculateStatistics, getReadings } from '../services/glucose.service.js'
import {
  analyzeEventOutcome,
...
export const analyzeTrendsTool = {
  name: 'analyze_trends',
  description:
    'Analyze glucose trends and patterns over time, including post-meal spikes, overnight patterns, and insulin effectiveness',
  inputSchema: z
    .object({
      days: z.number().positive().default(7).describe('Number of days to analyze (default: 7)'),
    })
    .strict(),
}
...
export async function compareExpectedVsActualHandler(args: {
  ...
  try {
    ...
    // Generate prediction
    let prediction: Awaited<ReturnType<typeof predictGlucoseImpact>>
              ...
              actual: {
                ...
                finalGlucose:
                  actualReadings.length > 0
                    ? actualReadings[actualReadings.length - 1].value
                    : null,
              },
              observation: outcome,
              disclaimer:
...
export const detectParameterDriftTool = {
  ...
  description:
    'Detect if your insulin sensitivity or carb ratio parameters appear to have changed over time',
  inputSchema: z
    .object({
