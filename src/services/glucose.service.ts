...
export async function getLatestReading(): Promise<GlucoseReading | null> {
  ...
  try {
    ...
    if (apiReadings.length > 0) {
      ...
      const ageMinutes = Math.floor(
        (Date.now() - new Date(apiReading.recordedAt).getTime()) / 1000 / 60,
      )
  ...
  try {
    ...
    if (shareReading) {
      const ageMinutes = Math.floor(
        (Date.now() - new Date(shareReading.recordedAt).getTime()) / 1000 / 60,
      )
      console.error(
        `[Share] Latest reading is ${ageMinutes} minutes old (${shareReading.recordedAt})`,
      )
    }
  } catch (error) {
  ...
  if (apiReading && shareReading) {
    ...
    if (shareTime > apiTime) {
      console.error(
        `✅ Using Share reading (${Math.floor((shareTime - apiTime) / 1000 / 60)} minutes newer)`,
      )
      return shareReading
    } else {
      console.error(
        `✅ Using API reading (${Math.floor((apiTime - shareTime) / 1000 / 60)} minutes newer)`,
      )
      return apiReading
    }
  }
  ...
  if (rangeDays > 90) {
    ...
    while (currentStart < end) {
      const currentEnd = new Date(
        Math.min(currentStart.getTime() + 90 * 24 * 60 * 60 * 1000, end.getTime()),
      )
...
export function calculateStatistics(readings: GlucoseReading[]): GlucoseStatistics {
  ...
  // Standard deviation
  const squaredDiffs = values.map((val) => (val - average) ** 2)
